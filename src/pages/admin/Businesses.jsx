    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map(i => (
            <div key={i} className="h-32 bg-secondary-light rounded-lg animate-pulse" />
          ))}
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col gap-1.5 mt-3 text-sm text-muted">
                    <span className="flex items-center gap-1.5">
                      <MapPin size={12} className="shrink-0" /> {business.direccion}
                    </span>
                    {business.phone && (
                      <span className="flex items-center gap-1.5">
                        <Phone size={12} className="shrink-0" /> {business.phone}
                      </span>
                    )}
                  </div>
                </div>
