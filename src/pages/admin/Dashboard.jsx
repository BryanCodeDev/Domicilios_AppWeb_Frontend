  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-muted text-sm mt-1">Metricas generales de la plataforma</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {recentOrders.map(order => (
                <div key={order.id} className="flex items-center justify-between py-3">
                  ...
                  <div className="text-right">
                    ...
                    <span className={"text-xs font-medium px-2 py-0.5 rounded-full " + (order.estado === "IN_TRANSIT" ? "bg-info-light text-info" : order.estado === "DELIVERED" ? "bg-success-light text-success" : "bg-warning-light text-warning")}>{order.estado}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-surface border border-subtle rounded-xl p-5">
          ...
          {ordersLoading ? (
            ...
            <div className="space-y-0 divide-y divide-subtle">
              {recentOrders.filter(o => o.business).slice(0, 3).map((order) => (
                <div key={order.id} className="flex items-center justify-between py-3">
